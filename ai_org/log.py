...
It is intentionally low-level and must not import patch series, patch, merge, or
reference modules.

Memento: one pull is one run. RunContext is created only at process entry
points such as patch_series pull, CLI mains, and test fixtures. Pipeline functions must
accept a caller context and derive child spans with ctx.child()/span. Do not
reintroduce fallback RunContext construction deep inside stage helpers: that
bug split one pull into many tiny run directories, which made tail/projections
lie about the real pipeline state. If a legacy direct-call path truly has no
caller context, keep the adapter boundary explicit and isolated.
"""
...
class RunContext:
    ...
    patch_series_id: str | None = None
    patch_series_serial: str | int | None = None
    patch_series_node_id: str | None = None
    stage: str | None = None
    step: str | None = None
    ...
    def __post_init__(self) -> None:
        ...
        correlation_id = self.correlation_id or self.request_id or self.patch_series_id or trace_id
...
def emit(
    ...
    *,
    ...
    with _WRITER_LOCK:
        ...
        for key in ("request_id", "patch_series_id", "patch_series_serial", "patch_series_node_id", "stage", "step", "attempt"):
...
def _project_events(
    ...
    *,
    ...
    for event in events:
        ...
        if event_type not in {"approach.step.started", "approach.step.completed", "approach.step.failed"}:
            continue
        payload = _payload(event)
        step = str(event.get("step") or payload.get("step") or "")
        if event_type == "approach.step.started":
            current_step = step or current_step
            continue
        if event_type == "approach.step.completed":
            seconds = float(payload.get("duration_seconds", payload.get("seconds", 0.0)) or 0.0)
            # Step name is duplicated on the record because large completed
            # payloads are artifact-backed and may lose inline payload.step.
            if step:
            ...
            current_step = payload.get("current_step") if "current_step" in payload else None
        elif event_type == "approach.step.failed":
            current_step = step or current_step
    ...
    for event in events:
        ...
        if event.get("attempt") is not None or payload.get("attempt") is not None:
            atom_fingerprints = payload.get("atom_fingerprints")
            if not isinstance(atom_fingerprints, list):
                atom_fingerprints = [payload.get("fingerprint") or payload.get("rejection_fingerprint")]
            atom_fingerprints = [fingerprint for fingerprint in atom_fingerprints if isinstance(fingerprint, str) and fingerprint]
            attempt_history.append(
                {
                    ...
                    "fingerprint": payload.get("fingerprint") or payload.get("rejection_fingerprint"),
                    "fingerprint_set": atom_fingerprints,
                    "repeated_fingerprints": payload.get("repeated_fingerprints"),
                    "defect_atoms": payload.get("defect_atoms"),
    ...
    if outcome is not None:
        ...
        if outcome.get("failure_mode"):
            run_status["failure_mode"] = outcome["failure_mode"]
    ...
    canonical = {"line": _canonical_line(run_status, progress_snapshot, events, timing_rows, attempt_history)}
    return {
...
def _canonical_line(
    ...
    failure_mode = f" failure_mode={run_status['failure_mode']}" if run_status.get("failure_mode") else ""
    repeated = _repeated_fingerprint_summary(attempt_history)
    fingerprint = f" repeated_fingerprint={repeated}" if repeated else ""
    return (
        f"{run_status.get('run_id')} {run_status.get('status')}{outcome}{failed_step}{failure_mode}{fingerprint} "
        f"events={run_status.get('event_count')} steps={done} current={current} seconds={duration:.3f}"
    )


def _repeated_fingerprint_summary(attempt_history: Sequence[Mapping[str, Any]]) -> str:
    counts: dict[str, int] = {}
    for attempt in attempt_history:
        fingerprints = attempt.get("fingerprint_set")
        if not isinstance(fingerprints, Sequence) or isinstance(fingerprints, (str, bytes)):
            fingerprints = [attempt.get("fingerprint")]
        for fingerprint in fingerprints:
            if not isinstance(fingerprint, str) or not fingerprint:
                continue
            counts[fingerprint] = counts.get(fingerprint, 0) + 1
    repeated = [(fingerprint, count) for fingerprint, count in counts.items() if count > 1]
    if not repeated:
        return ""
    fingerprint, count = sorted(repeated, key=lambda item: (-item[1], item[0]))[0]
    return f"{fingerprint}x{count}"
...
def _has_success_terminal(events: Sequence[Mapping[str, Any]]) -> bool:
    ...
    return any(event_type in {"run.completed", "patch_series.produce.completed"} for event_type in terminal) or any(
...
def _latest_outcome(events: Sequence[Mapping[str, Any]]) -> dict[str, Any] | None:
    for event in reversed(events):
        if str(event.get("event_type", "")) != "patch_series.pull.outcome":
            continue
        payload = _payload(event)
        status = payload.get("status")
