...
the precedent store is org-level state, not work-repo state. Entries are
...
from ai_org.codex_reset import codex_resume_command, run_with_reset_retries


LOGGER = logging.getLogger(__name__)
REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_STORE_PATH = Path.home() / "aiorg_assets" / "engineering_precedent_store" / "org.sqlite3"
_REFERENCE_LOG_CONTEXT: contextvars.ContextVar[org_log.RunContext | None] = contextvars.ContextVar(
    "ai_org_precedent_log_context",
    default=None,
)
...
UNCLASSIFIED_EVIDENCE_CLASS = "unclassified"
...
ORG_PREHELD_LESSONS = (
    ...
    },
    {
        ...
        "facet": {
            ...
            "when_to_use": "Use this separation when branch-local metadata controls recursive traversal, parent-child traversal, or ownership decisions.",
    ...
    },
    {
        ...
        "kind": "design",
        "facet": {
            "structure": "Lineage parent trees must merge child implementation branches, not child patch series branches. Child patch series branches are doc nodes that carry sibling-local patch-series-cover-letter.json, technical-approach-plan.json, and patch-series-metadata.json at identical paths, so merging more than one child doc node into the same parent creates structural path conflicts.",
            "rationale": "A live smoke run hit a merge conflict in technical-approach-plan.json when sibling patch series doc branches were treated as the integration artifact.",
            "when_to_use": "Use this rule when determining parent-child resolution, parent rollout readiness, or any group-tree merge policy for split patch series children.",
            "when_not_to_use": "Do not require a child patch series branch itself to be an ancestor of its parent patch series, because that makes metadata occupy one shared implementation path.",
            ...
            "alternatives": "Merging child patch series branches into the parent was rejected because sibling metadata files collide at fixed paths.",
            "implementation_hooks": "Treat acceptance: passed or acceptance: reachable on either the child patch series branch or ai-org/contrib/<child-id> as the acceptance marker, then require the contrib branch to be an ancestor of the parent patch series.",
            "quality_attributes": "Lineage correctness, merge reliability, metadata isolation.",
            "evidence": "ai-org-bootstrap-codex@this-commit; live smoke sibling patch series merge conflict in technical-approach-plan.json.",
            "delta_claim": "Group trees integrate implementation contrib branches while child patch series branches remain isolated metadata nodes.",
    ...
    {
        "term": "vector figure base construction",
        "kind": "design",
        "facet": {
            "structure": "Construct the character from reusable base primitives before styling: head, neck, torso, pelvis, arms, and legs are explicit grouped masses with shared anchors, mirrored limb logic, and proportion variables. Outfit, hair, and accessories attach to the base instead of replacing it.",
            "rationale": "Professional vector character systems such as Open Peeps and Humaaans keep figures coherent by recombining consistent body bases with swappable parts.",
            "when_to_use": "Use for any SVG character, mascot, person, or humanoid figure that must survive regeneration, rigging, or variant production.",
            "when_not_to_use": "Do not use a base-only canon for pure logos, objects, abstract marks, or non-figure assets where body anchors would invent irrelevant anatomy.",
            "tradeoffs": "A base skeleton costs setup time and can feel plain until styled, but it prevents costume pieces and poses from drifting away from the same figure.",
            "alternatives": "One-off freehand silhouettes were rejected after amateur-corpus exemplars produced poisoned, non-constructive figure output.",
            "implementation_hooks": "Define body-group ids and base measurements first, then draw clothing and details as children or overlays keyed to those anchors.",
            "quality_attributes": "Constructive coherence, riggability, variant reuse, proportion stability.",
            "evidence": "anchor:pro-corpus-2026-07-03; professional vector figure systems: Open Peeps and Humaaans.",
            "delta_claim": "Vector figures need an explicit base construction layer before costume and style details.",
            "source_url": "ai-org-bootstrap-codex@pro-corpus-2026-07-03#open-peeps-humaaans",
            "evidence_class": "practitioner_report",
        },
    },
    {
        "term": "vector costume layering",
        "kind": "design",
        "facet": {
            "structure": "Layer clothing as separate vector shapes over the body base in a stable order: skin/base, lower garments, upper garments, outerwear, hair, props, and foreground accents. Each layer should follow underlying limb and torso anchors so the outfit reads as worn by the figure.",
            "rationale": "Open Peeps and Humaaans demonstrate professional reuse through swappable outfit layers that preserve the same base character construction.",
            "when_to_use": "Use when a figure has clothes, armor, uniforms, props, or hair that must remain editable and visually attached to the body.",
            "when_not_to_use": "Do not flatten costume and body into one ambiguous blob when the asset needs rigging, variants, or clean occlusion.",
            "tradeoffs": "Layered parts increase SVG group count, but they make occlusion, recoloring, and animation predictable.",
            "alternatives": "Merged decorative contours were rejected because they hide body structure and make costume variants fragile.",
            "implementation_hooks": "Create costume groups after base groups, clip or mask only where necessary, and keep garment edges aligned to shoulder, waist, elbow, knee, and ankle anchors.",
            "quality_attributes": "Editability, occlusion clarity, variant reuse, animation readiness.",
            "evidence": "anchor:pro-corpus-2026-07-03; professional vector figure systems: Open Peeps and Humaaans.",
            "delta_claim": "Costume detail should be an anchored layer system, not freehand decoration that replaces construction.",
            "source_url": "ai-org-bootstrap-codex@pro-corpus-2026-07-03#open-peeps-humaaans",
            "evidence_class": "practitioner_report",
        },
    },
    {
        "term": "vector character value staging",
        "kind": "design",
        "facet": {
            "structure": "Stage figure readability with a deliberate value stack: dominant body or outfit value, contrasting face and hand values, controlled accent colors, and enough background-free separation for small-size recognition. Reserve the highest contrast for identity-bearing features.",
            "rationale": "Professional vector figure libraries keep characters readable through restrained palettes and contrast placement rather than many local details.",
            "when_to_use": "Use for character SVGs that must read at icon, sticker, mascot, or small illustration sizes.",
            "when_not_to_use": "Do not add equal-contrast detail everywhere; it creates noise and competes with the face, pose, or defining prop.",
            "tradeoffs": "A constrained value hierarchy may reduce texture richness, but it improves recognition and downstream compositing.",
            "alternatives": "Uniformly detailed amateur exemplars were rejected because they buried figure identity in decorative noise.",
            "implementation_hooks": "Assign fill values before detail drawing, test the asset at small size, and keep accents subordinate unless they are the character's defining mark.",
            "quality_attributes": "Small-size legibility, figure-ground separation, identity focus, palette discipline.",
            "evidence": "anchor:pro-corpus-2026-07-03; professional vector figure systems: Open Peeps and Humaaans.",
            "delta_claim": "Vector character construction needs value staging so identity features survive simplification.",
            "source_url": "ai-org-bootstrap-codex@pro-corpus-2026-07-03#open-peeps-humaaans",
            "evidence_class": "practitioner_report",
        },
    },
    {
        "term": "vector silhouette first construction",
        "kind": "design",
        "facet": {
            "structure": "Solve the figure as a readable outer silhouette before internal features: pose line, head mass, torso mass, limb angles, negative spaces, and major prop outline must communicate the character without relying on facial details or texture.",
            "rationale": "Professional modular character systems make poses and parts interchangeable because the silhouette and major masses carry the read before surface detail.",
            "when_to_use": "Use for mascots, avatars, game characters, and editorial figures where pose and identity must remain legible at reduced sizes.",
            "when_not_to_use": "Do not depend on tiny facial marks, texture, or costume seams to distinguish the figure if the silhouette is generic.",
            "tradeoffs": "Silhouette-first work can delay decoration, but it exposes weak poses and tangents before detail hides them.",
            "alternatives": "Detail-first amateur construction was rejected after it produced incoherent figures whose outlines did not carry the design.",
            "implementation_hooks": "Block the outer contour and negative spaces first, check it in one color, then add internal groups only after the silhouette reads.",
            "quality_attributes": "Instant recognition, pose clarity, small-size robustness, tangent control.",
            "evidence": "anchor:pro-corpus-2026-07-03; professional vector figure systems: Open Peeps and Humaaans.",
            "delta_claim": "Vector figure output should pass a silhouette read before internal costume or face detail is trusted.",
            "source_url": "ai-org-bootstrap-codex@pro-corpus-2026-07-03#open-peeps-humaaans",
            "evidence_class": "practitioner_report",
        },
    },
...
def build_search_keywords_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
        "required": ["keywords"],
...
def build_design_search_keywords_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
        "required": ["keywords"],
...
def build_extract_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
        "required": ["relevant", "snippet", "summary", "lang_env_version", "pitfalls", "evidence_class"],
        "properties": {
...
def build_design_source_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
        "required": ["sources"],
...
def build_design_extract_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
        "required": [
...
def build_delta_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
...
def build_design_competence_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
...
def build_distill_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
...
def build_author_level_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
...
def build_precedent_terms_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
        "required": ["terms"],
...
_SCHEMA_BUILDERS = {
    "AUTHOR_LEVEL_SCHEMA": build_author_level_schema,
    "BASELINE_SCHEMA": build_baseline_schema,
    "DELTA_SCHEMA": build_delta_schema,
    "DESIGN_COMPETENCE_SCHEMA": build_design_competence_schema,
    "DESIGN_EXTRACT_SCHEMA": build_design_extract_schema,
    "DESIGN_SEARCH_KEYWORDS_SCHEMA": build_design_search_keywords_schema,
    "DESIGN_SOURCE_SCHEMA": build_design_source_schema,
    "DISTILL_SCHEMA": build_distill_schema,
    "EXTRACT_SCHEMA": build_extract_schema,
    "REFERENCE_TERMS_SCHEMA": build_precedent_terms_schema,
    "SEARCH_KEYWORDS_SCHEMA": build_search_keywords_schema,
}
...
# Paraphrase-tolerant read layer (ratified 2026-07-04): reads may match stored
# knowledge for reworded queries so wording drift stops triggering duplicate
# research. Implementation note: the CURRENT mechanism is order-insensitive
# token overlap; that is one implementation of paraphrase retrieval, not the
# concept itself (brief 30: naming the layer after the mechanism prompted a
# bag-of-tokens-only evolution path).
# Both floors are PROVISIONAL: a candidate key is eligible
# only when at least this fraction of the query tokens appears in the stored
# key AND - only when the stored key has FEWER tokens than the query - at
# least this fraction of the stored key tokens appears in the query. The
# stored-side floor is DIRECTIONAL (brief 30): its documented purpose is to
# stop a SHORT stored key from hijacking a LONGER query, so it must not fire
# when the stored key is longer or equal - applied symmetrically it made the
# 4-token query 'defer implementation question routing' MISS the 7-token
# canon key 'defer design question to implementation routing criteria' at
# 4/7=0.57 despite every query token matching (live probe, 2026-07-05).
# 0.6 = strictly more than half: at 0.5, "unknown canon" fuzzy-hit
# "known canon" on the single shared generic token, which is a false positive.
PARAPHRASE_MIN_QUERY_TOKEN_FRACTION = 0.6
PARAPHRASE_MIN_STORED_TOKEN_FRACTION = 0.6
# Morphological family tokens (brief 30, ratified 2026-07-05): alongside each
# original token, the DERIVED index and the query tokenization also emit a
# family token produced by this FROZEN, ordered, deterministic English
# suffix-strip list. First suffix whose stem passes the minimum-stem guard
# wins; at most one suffix is ever stripped; ASCII-alpha tokens only; original
# tokens are always retained (family tokens are additive). No Porter stemmer,
# no language detection. The list and the guard are PROVISIONAL choices; 'up'
# covers compound verb nouns (cleanup/clean, lookup/look), which the live
# probe 'terminal prompt cleanup on exit' vs stored 'clean terminal prompt
# restoration' required (2026-07-05). Memento: keys are NEVER touched -
# families live only in the rebuildable knowledge_term_tokens layer.
# Any change to the suffix list or the stem guard changes
# PARAPHRASE_FAMILY_RULES_VERSION below, which triggers a full derived-index
# rebuild on the next store open (the stamp is derived from the rules, so a
# rule edit cannot forget to bump it).
PARAPHRASE_FAMILY_SUFFIXES = ("ation", "tion", "ment", "ing", "es", "ed", "up", "s")
PARAPHRASE_FAMILY_MIN_STEM_LENGTH = 4  # PROVISIONAL over-stripping guard
PARAPHRASE_FAMILY_RULES_VERSION = (
    "family-v1:" + "+".join(PARAPHRASE_FAMILY_SUFFIXES) + f":min-stem-{PARAPHRASE_FAMILY_MIN_STEM_LENGTH}"
)
REFERENCE_MAX_PARALLEL = 6
REFERENCE_SQLITE_BUSY_TIMEOUT_MS = 30000
# Memento (requester 2026-07-16): relaxed 24h -> 90d as an interim fix. The 24h
# TTL re-extracted stable engineering knowledge (candidates rows persist and do
# not go stale — a 1960s paper does not change) every day: reading the SQLite
# entry then searching anyway = same freshness/staleness GIGO class removed in
# impl3. Time-based invalidation is wrong for a reference/knowledge cache; the
# real fix is source-stability-based invalidation (hash/etag) + persisting the
# "nothing-to-add" negative conclusion. 90d is the stopgap until that surgery.
REFERENCE_RESEARCH_TTL_SECONDS = 7776000  # 90 days
GH_SEARCH_WINDOW_SECONDS = 60.0
WEB_SEARCH_WINDOW_SECONDS = 60.0


class ReferenceCodexTimeout(TimeoutError):
    """Raised when a bounded precedent-store Codex subprocess exceeds its timeout."""


class StoreUnavailable(RuntimeError):
    """Raised when the precedent store cannot be queried or written.

    Memento: empty vs unreadable vs unset are three different nulls. Returning
    failure as empty silenced three consecutive dispatch incidents and one false
    code comment; unavailable is now loud by default, and fail-open exists only
    as written catches.
    """

    VALID_REASONS = {"unset", "missing", "unreadable"}

    def __init__(self, reason: str, detail: Any = "") -> None:
        clean_reason = str(reason or "").strip()
        if clean_reason not in self.VALID_REASONS:
            raise ValueError(f"unknown store unavailable reason: {clean_reason}")
...
def _env_float(name: str, default: float) -> float:
    try:
        requested = float(os.environ.get(name, ""))
    except ValueError:
        return default
    return requested if requested > 0 else default


GH_SEARCH_PER_MIN = _env_int("AI_ORG_GH_SEARCH_PER_MIN", 18)
WEB_SEARCH_PER_MIN = _env_int("AI_ORG_WEB_SEARCH_PER_MIN", 12)
GH_SEARCH_RATE_LIMIT_MAX_WAIT_SECONDS = 90.0
GH_SEARCH_CACHE_TTL_SECONDS = 86400.0
GH_SEARCH_FILE_LOCK_TIMEOUT_SECONDS = 5.0
GITHUB_SEARCH_BOOLEAN_OPERATOR_LIMIT = 5
DESIGN_REPO_SEARCH_TERMS = ("ADR", "patch series", "KEP", "PEP", "DESIGN.md", "architecture", "docs")
...
_BACKGROUND_BUILD_EXECUTOR = _DaemonThreadPoolExecutor(
    max_workers=1,
    thread_name_prefix="ai-org-precedent-store-background",
)


@contextmanager
def _precedent_log_context(ctx: org_log.RunContext | None):
    token = _REFERENCE_LOG_CONTEXT.set(ctx)
    try:
        yield
    finally:
        _REFERENCE_LOG_CONTEXT.reset(token)


def _precedent_ctx(stage: str, ctx: org_log.RunContext | None = None) -> org_log.RunContext:
    current = ctx or _REFERENCE_LOG_CONTEXT.get()
    if current is not None:
        return current.child(stage=stage)
    return org_log.RunContext(repo=REPO_ROOT, stage=stage)
...
_PARAPHRASE_SPLIT_PATTERN = re.compile(r"[\W_]+", re.UNICODE)


def _paraphrase_tokens(text: str) -> tuple[str, ...]:
    """Tokenize text for paraphrase-tolerant retrieval.

    Implementation note (frozen W1 tokenizer): NFC normalize + casefold +
    whitespace/punctuation split.

    Memento: this is a DERIVED read-side layer only. It never mutates stored
    rows, and it deliberately has no stemming and no synonym expansion so that
    token identity is deterministic across runs and sqlite builds. Do not add
    linguistic cleverness here; the separate, rebuildable layer for that is
    _paraphrase_family_token (frozen suffix rules, additive index rows,
    version-stamped) - never the tokenizer itself.
    """
    normalized = unicodedata.normalize("NFC", str(text or "")).casefold()
    return tuple(sorted({token for token in _PARAPHRASE_SPLIT_PATTERN.split(normalized) if token}))


def _paraphrase_family_token(token: str) -> str:
    """Return the morphological family token for token, or '' when none applies.

    Frozen rule (brief 30): ASCII-alpha tokens only; iterate the pinned
    PARAPHRASE_FAMILY_SUFFIXES in order; the FIRST suffix whose stripped stem
    meets PARAPHRASE_FAMILY_MIN_STEM_LENGTH wins (a suffix whose stem is too
    short does not consume the token - later suffixes still get their turn);
    at most one suffix is stripped. Deterministic: no randomness, no locale,
    no library stemmer. Changing anything here changes
    PARAPHRASE_FAMILY_RULES_VERSION and forces a derived-index rebuild.
    """
    if not (token.isascii() and token.isalpha()):
        return ""
    for suffix in PARAPHRASE_FAMILY_SUFFIXES:
        if token.endswith(suffix):
            stem = token[: -len(suffix)]
            if len(stem) >= PARAPHRASE_FAMILY_MIN_STEM_LENGTH:
                return stem
    return ""


def _paraphrase_candidate_keys(connection: sqlite3.Connection, term: str) -> list[str]:
    """Rank stored term keys that plausibly paraphrase the query term.

    Implementation note: the CURRENT paraphrase mechanism is order-insensitive
    token overlap over originals plus additive morphological family tokens.
    Ranking is (a) fraction of query tokens matched (exact or family),
    descending, then (b) count of exactly-matched query tokens, descending -
    an exact token match outranks a family-only match - then (c) term_key
    byte order as the deterministic tiebreak. No randomness and no time
    dependence. Eligibility floors are PROVISIONAL, see the PARAPHRASE_*
    constants.
    """
    query_tokens = _paraphrase_tokens(term)
    if not query_tokens:
        return []
    # Each lookup token maps back to the ORIGINAL query tokens it can match
    # for; fractions are always over original query tokens, never over the
    # family-expanded set (expansion must not dilute the floors).
    lookup_map: dict[str, set[str]] = {}
    for token in query_tokens:
        lookup_map.setdefault(token, set()).add(token)
        family = _paraphrase_family_token(token)
        if family:
            lookup_map.setdefault(family, set()).add(token)
    placeholders = ",".join("?" for _ in lookup_map)
    rows = connection.execute(
        f"""
        SELECT term_key, token, kind, source_token
        FROM knowledge_term_tokens
        WHERE token IN ({placeholders})
        """,
        list(lookup_map),
    ).fetchall()
    matched_query: dict[str, set[str]] = {}
    exact_query: dict[str, set[str]] = {}
    matched_stored: dict[str, set[str]] = {}
    for row in rows:
        term_key = str(row["term_key"] or "")
        token = str(row["token"] or "")
        origins = lookup_map.get(token, set())
        matched_query.setdefault(term_key, set()).update(origins)
        # source_token is the stored ORIGINAL token that emitted this index
        # row, so the stored-side fraction stays over stored originals.
        matched_stored.setdefault(term_key, set()).add(str(row["source_token"] or ""))
        # Exact means: a stored ORIGINAL token is itself a query original.
        # A stored 'exact' row hit through a query FAMILY token, or a stored
        # 'family' row hit by a query original, both stay family-level.
        if str(row["kind"] or "") == "exact" and token in origins:
            exact_query.setdefault(term_key, set()).add(token)
    ranked: list[tuple[float, int, str]] = []
    for term_key, matched in matched_query.items():
        total = int(
            connection.execute(
                "SELECT COUNT(*) FROM knowledge_term_tokens WHERE term_key = ? AND kind = 'exact'",
                (term_key,),
            ).fetchone()[0]
        )
        query_fraction = len(matched) / len(query_tokens)
        stored_matched = len(matched_stored.get(term_key, set()))
        stored_fraction = stored_matched / total if total else 0.0
        if query_fraction < PARAPHRASE_MIN_QUERY_TOKEN_FRACTION:
            continue
        # Memento (brief 30, directional floor): the stored-side floor only
        # guards the short-stored-key-hijacks-long-query direction. When the
        # stored key is longer or equal, the query-side floor alone governs;
        # see the PARAPHRASE_MIN_* constants comment for the live false miss
        # that symmetric application caused.
        if total < len(query_tokens) and stored_fraction < PARAPHRASE_MIN_STORED_TOKEN_FRACTION:
            continue
        ranked.append((query_fraction, len(exact_query.get(term_key, set())), term_key))
    ranked.sort(key=lambda item: (-item[0], -item[1], item[2]))
    return [key for _fraction, _exact, key in ranked]
...
def lookup(term: str, context: Mapping[str, Any] | None = None, kind: str | None = None) -> dict[str, Any] | None:
    ...
    Memento (ratified 2026-07-04): write-exact / paraphrase-tolerant reads /
    keys never destroyed.
    - Writes store the caller's term string verbatim. Stored keys are
      provenance; they are never rewritten, merged, or normalized in place,
      and write-side dedup stays byte-identical only.
    - Reads resolve exact first (short-circuit), then fall back to
      paraphrase-tolerant retrieval over a derived index (implementation
      note: order-insensitive token overlap; frozen W1 tokenizer: NFC +
      casefold + whitespace/punctuation split; plus additive morphological
      family tokens from the frozen PARAPHRASE_FAMILY_* rules - no synonyms,
      no Porter stemmer). Reworded queries must find existing knowledge
      instead of triggering duplicate research (requester: two runs
      re-researching the same term because the wording drifted).
    - The paraphrase layer applies to knowledge reads only. The gh-search
      memo cache stays exact-keyed, and no read path ever mutates stored rows.
    - AI_ORG_PRECEDENT_READ_DISABLED=1 makes every knowledge read report a
      miss (emergence mode) while all writes keep persisting as normal.
    """
    entry = _read_entry(term, include_deprecated=False)
    if entry is None:
        entry = _empty_public_research_entry(term)
...
def audit(term: str) -> dict[str, Any] | None:
    ...
    Audit stays on the exact key layer (paraphrase=False): it is a provenance
    inspection surface, and a fuzzy hit would silently show another term's
    maintenance record. Knowledge retrieval fuzziness lives in lookup().
    """
    entry = _read_entry(term, include_deprecated=True, paraphrase=False)
    if entry is None:
        return _empty_public_research_entry(term)
...
def query(filters: Mapping[str, Any] | None = None) -> list[dict[str, str]]:
    ...
    # Knowledge read path: reports miss under AI_ORG_PRECEDENT_READ_DISABLED.
    if _precedent_read_disabled():
        return []
    filters = dict(filters or {})
    path = _existing_database_path()
    ...
    if search_values:
        where.append(
            ...
            + " OR ".join(
                ...
                "WHERE r.term_key = c.term_key "
                "AND r.deprecated_at IS NULL "
                "AND lower(r.search_keywords) LIKE lower(?)"
                ")"
                ")"
    ...
    sql = (
        ...
        "c.lang_env_version, c.author_level, c.source_url, c.found_via, c.evidence_class, "
        "c.deprecated_at, c.deprecated_reason "
        "FROM candidates c"
    )
    if where:
    ...
    try:
        ...
        raise StoreUnavailable("unreadable", _store_error_detail(path, exc)) from exc
...
def seed_preheld_org_lessons() -> dict[str, Any]:
    """Idempotently seed hard-won org lessons into the configured precedent store."""
...
def deprecate_term(term: str, reason: str) -> dict[str, Any]:
    """Soft-deprecate all research and candidates for a term."""
    return _deprecate_rows(term, reason, include_research=True)


def deprecate_candidates(term: str, reason: str) -> dict[str, Any]:
    """Soft-deprecate stored candidates for a term while preserving research history."""
    return _deprecate_rows(term, reason, include_research=False)
...
def expand(
    ...
    *,
    ...
    log_ctx = _precedent_ctx("engineering_precedent_store.expand", ctx).child(step=str(term))
    with _precedent_log_context(log_ctx), org_log.span("engineering_precedent_store.expand", log_ctx) as expand_ctx:
        result = _expand(term, context, force=force, kinds=kinds, ctx=expand_ctx)
        org_log.emit(
            "engineering_precedent_store.expand.result",
            {
                "term": term,
                "kinds": list(_precedent_kinds(kinds)),
                "candidates": len(result.get("candidates", [])) if isinstance(result.get("candidates"), list) else 0,
...
def _expand(
    ...
    *,
    ...
    requested_kinds = _precedent_kinds(kinds)
    context = dict(context or {})
    context.setdefault("_precedent_codex_timeouts", [])
    if not force and not _precedent_force_enabled():
    ...
    if "implementation" in requested_kinds:
        ...
        implementation_fetched = fetch_candidates(
            ...
            {
                **context,
                "_precedent_search_keywords": search_keywords,
                "_precedent_examined": examined,
            },
        )
    ...
    if "design" in requested_kinds:
        design_fetched = fetch_design_candidates(
            ...
            {
                **context,
                "_precedent_design_search_keywords": design_keywords,
                "_precedent_examined": examined,
            },
        )
    ...
    # Memento: band-aids never stand alone because no primary source prescribes
    # the band-aid; the evidence is asymmetric with the mature pattern it works
    # around. Demotion keys on source type only, never on content wording.
    kept = _fold_workaround_candidates(term, kept, ctx)
    low_level_count = sum(1 for candidate in kept if _is_low_level_author(candidate.get("author_level", "")))
    ...
    if context["_precedent_codex_timeouts"]:
        timeout_note = "timed-out: " + "; ".join(context["_precedent_codex_timeouts"])
...
def build_from_patch_series(
    ...
    *,
    ...
    """Build precedent entries for implementation-bearing terms found in a patch series-shaped view."""
    log_ctx = _precedent_ctx("engineering_precedent_store.build_from_patch_series", ctx)
    with _precedent_log_context(log_ctx), org_log.span("engineering_precedent_store.build_from_patch_series", log_ctx) as build_ctx:
        result = _build_from_patch_series(patch_series_view, context, force=force, kinds=kinds, ctx=build_ctx)
        org_log.emit(
            "engineering_precedent_store.build_from_patch_series.result",
            {
                "processed_terms": result.get("processed_terms"),
                "expanded": result.get("expanded"),
                "hits": result.get("hits"),
                "failed": result.get("failed"),
            },
            ctx=build_ctx,
            severity="warning" if result.get("failed") else "info",
        )
        return result


def _build_from_patch_series(
    patch_series_view: Mapping[str, Any],
    context: Mapping[str, Any] | None = None,
    force: bool = False,
    kinds: Any | None = None,
    *,
    ...
    requested_kinds = _precedent_kinds(kinds)
    context = dict(context or {})
    text = _patch_series_text(patch_series_view)
    built: dict[str, Any] = {}
    expanded: list[str] = []
    hits: list[str] = []
    failed: dict[str, str] = {}

    terms = _dedupe_terms_by_key(_extract_precedent_terms(text, context))
    if not terms and context.get("_precedent_codex_timeouts"):
        failed["__term_extraction__"] = "; ".join(context["_precedent_codex_timeouts"])
        return {
            "terms": built,
            "processed_terms": [],
            "expanded": expanded,
    ...
    parallelism = _precedent_parallelism(len(terms))
    if parallelism <= 1:
        for term in terms:
            outcomes[term] = _build_patch_series_term(term, context, force, requested_kinds, ctx=ctx.child(step=term))
    else:
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallelism) as executor:
            futures = {
                executor.submit(_build_patch_series_term, term, context, force, requested_kinds, ctx=ctx.child(step=term)): term
                for term in terms
            }
...
def _build_patch_series_term(
    ...
    *,
    ...
    try:
        log_ctx = _precedent_ctx("engineering_precedent_store.term", ctx).child(step=term)
        with _precedent_log_context(log_ctx):
            return _build_patch_series_term_logged(term, context, force=force, kinds=kinds, ctx=log_ctx)
    except Exception as exc:
        return {"status": "failed", "error": _format_term_error(exc)}


def _build_patch_series_term_logged(
    term: str,
    context: Mapping[str, Any],
    force: bool,
    ...
    *,
    ...
    try:
        ...
        return {"status": "expanded", "entry": _call_expand(term, context, force=force, kinds=requested_kinds, ctx=ctx)}
    except Exception as exc:
        return {"status": "failed", "error": _format_term_error(exc)}
...
def start_background_build(
    ...
    *,
    ...
    """Start a non-blocking precedent-store build and return its Future."""
    build_context = dict(context or {})
    requested_kinds = _precedent_kinds(kinds)
    log_ctx = _precedent_ctx("engineering_precedent_store.background_build", ctx)
    org_log.emit(
        "engineering_precedent_store.background_build.scheduled",
        {"kinds": list(requested_kinds)},
        ctx=log_ctx,
    )
    future = _BACKGROUND_BUILD_EXECUTOR.submit(
        _background_build_from_patch_series,
        dict(patch_series_view),
        build_context,
        force,
        requested_kinds,
        log_ctx,
...
def await_background_builds(timeout: float | None = None) -> None:
    """Wait for currently outstanding background precedent-store builds to finish."""
...
def _background_build_from_patch_series(
    patch_series_view: Mapping[str, Any],
    context: Mapping[str, Any],
    force: bool,
    kinds: tuple[str, ...],
    ...
    try:
        log_ctx = _precedent_ctx("engineering_precedent_store.background_build", ctx)
        with _precedent_log_context(log_ctx), org_log.span("engineering_precedent_store.background_build", log_ctx) as background_ctx:
            return _call_build_from_patch_series(patch_series_view, context, force=force, kinds=kinds, ctx=background_ctx)
    except Exception as exc:
        error = _format_term_error(exc)
        LOGGER.exception("background precedent-store build failed: %s", error)
        if ctx is not None:
            org_log.debug_emit("engineering_precedent_store.background_build.failed", {"error": error}, ctx=ctx, severity="error")
        return {"ok": False, "error": error, "failed": {"__background_build__": error}}


def _call_build_from_patch_series(
    patch_series_view: Mapping[str, Any],
    context: Mapping[str, Any],
    *,
    force: bool,
    ...
    kwargs: dict[str, Any] = {"force": force, "kinds": kinds}
    try:
        parameters = inspect.signature(build_from_patch_series).parameters
    except (TypeError, ValueError):
        parameters = {}
    if "ctx" in parameters:
        kwargs["ctx"] = ctx
    return build_from_patch_series(patch_series_view, context, **kwargs)
...
def _call_expand(
    ...
    *,
    ...
    if "ctx" in parameters:
        kwargs["ctx"] = ctx
    return expand(term, context, **kwargs)


def _precedent_kinds(kinds: Any | None) -> tuple[str, ...]:
    if kinds is None:
    ...
    for value in raw_values:
        ...
        if kind not in CANDIDATE_KINDS:
            raise ValueError(f"invalid precedent kind: {value!r}")
        cleaned.add(kind)
    if not cleaned:
        raise ValueError("precedent kinds must not be empty")
...
def _lookup_term_for_kinds(
    ...
    entry = _read_entry(term, missing_ok=True, include_deprecated=False)
...
def _precedent_parallelism(term_count: int) -> int:
    if term_count <= 1:
        return 1
...
def _precedent_research_ttl_seconds() -> float:
...
def _precedent_force_enabled() -> bool:
    raw = os.environ.get("AI_ORG_REFERENCE_FORCE")
    return raw is not None and raw.strip().lower() not in {"", "0", "false", "no", "off"}


def _precedent_read_disabled() -> bool:
    """Exploration switch (ratified brief13): AI_ORG_PRECEDENT_READ_DISABLED=1.

    Memento: when set, every knowledge read path reports a miss so the org is
    forced into fresh research (emergence mode), while every write path keeps
    persisting exactly as normal - everything learned during the run is
    banked. The gh rate limiter and the memo cache WRITE side are never gated
    on this flag; only reads are.
    """
    raw = os.environ.get("AI_ORG_PRECEDENT_READ_DISABLED")
    return raw is not None and raw.strip().lower() not in {"", "0", "false", "no", "off"}


def _precedent_search_timeout_seconds() -> float:
...
def _record_precedent_timeout(context: Mapping[str, Any], exc: BaseException) -> None:
    if not isinstance(context, dict):
        return
    timeouts = context.setdefault("_precedent_codex_timeouts", [])
    if isinstance(timeouts, list):
        timeouts.append(str(exc))


def _recent_research_entry(term: str, kinds: tuple[str, ...]) -> dict[str, Any] | None:
    ttl = _precedent_research_ttl_seconds()
    if ttl <= 0:
        return None
...
def fetch_candidates(term: str, context: Mapping[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    """
    context = dict(context or {})
    search_keywords = context.pop("_precedent_search_keywords", None)
    examined = context.pop("_precedent_examined", None)
            ...
            candidates.append(
                {
                    ...
                    "evidence_class": _clean_evidence_class(extracted.get("evidence_class")),
                    "found_via": found_via,
                    "_precedent_repo": full_name,
                    "_precedent_language": primary_language,
                    "_precedent_found_via": found_via,
                }
            )
...
def fetch_design_candidates(term: str, context: Mapping[str, Any] | None = None) -> list[dict[str, Any]]:
    """Fetch public design candidates from design docs, ADRs/patch seriess, and web design sources."""
    context = dict(context or {})
    design_keywords = context.pop("_precedent_design_search_keywords", None)
    examined = context.pop("_precedent_examined", None)
...
def _codex_search_keywords(term: str, context: Mapping[str, Any]) -> list[str]:
    try:
        result = _codex_json(
            ...
            build_search_keywords_schema(),
            "search-keywords.json",
        )
    except ReferenceCodexTimeout as exc:
        _record_precedent_timeout(context, exc)
        return []
...
def _codex_design_search_keywords(term: str, context: Mapping[str, Any]) -> list[str]:
    try:
        result = _codex_json(
            ...
            build_design_search_keywords_schema(),
...
def _search_repositories(keywords: list[str], context: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    seen: set[str] = set()
    for query, found_via in _github_repo_search_queries(keywords):
        cmd = [
            "gh",
            "search",
            "repos",
            ...
            "5",
            "--json",
            "fullName,url,description,language,stargazersCount",
        ]
        ...
        for repo in raw_repos:
            ...
            normalized["found_via"] = found_via
...
def _search_design_repositories(keywords: list[str], context: Mapping[str, Any]) -> list[dict[str, Any]]:
    repos: list[dict[str, Any]] = []
    seen: set[str] = set()
    for query, found_via in _github_repo_search_queries(keywords, refinements=DESIGN_REPO_SEARCH_TERMS):
        cmd = [
            "gh",
            "search",
            "repos",
...
def _github_repo_search_queries(
    keywords: list[str],
    *,
    refinements: tuple[str, ...] = (),
) -> list[tuple[str, str]]:
    # Memento: external API contracts are enforced at construction. In
    # run-20260703T010030Z-0655f04e, 482 gh search repos subprocesses failed
    # deterministically because each query used more than GitHub's five
    # AND/OR/NOT operators; such a query is never worth a retry.
    queries: list[tuple[str, str]] = []
    for keyword in keywords:
        found_via = " ".join(str(keyword or "").strip().split())
        if not found_via:
            continue
        for keyword_query in _split_github_boolean_query(found_via):
            if not refinements:
                queries.append((keyword_query, found_via))
                continue
            operator_count = _github_search_boolean_operator_count(keyword_query)
            max_refinement_terms = max(1, GITHUB_SEARCH_BOOLEAN_OPERATOR_LIMIT - operator_count + 1)
            for chunk in _chunks(list(refinements), max_refinement_terms):
                query = f"{keyword_query} {' OR '.join(chunk)}"
                if _github_search_boolean_operator_count(query) <= GITHUB_SEARCH_BOOLEAN_OPERATOR_LIMIT:
                    queries.append((query, found_via))
                else:
                    queries.extend((f"{keyword_query} {term}", found_via) for term in chunk)
    return queries


def _split_github_boolean_query(query: str) -> list[str]:
    cleaned = " ".join(str(query or "").strip().split())
    if not cleaned:
        return []
    if _github_search_boolean_operator_count(cleaned) <= GITHUB_SEARCH_BOOLEAN_OPERATOR_LIMIT:
        return [cleaned]
    if not re.search(r"\b(?:AND|NOT)\b", cleaned):
        parts = [part.strip() for part in re.split(r"\s+\bOR\b\s+", cleaned) if part.strip()]
        if parts:
            return [" OR ".join(chunk) for chunk in _chunks(parts, GITHUB_SEARCH_BOOLEAN_OPERATOR_LIMIT + 1)]
    operands = [
        part.strip()
        for part in re.split(r"\s+\b(?:AND|OR|NOT)\b\s+", cleaned)
        if part.strip()
    ]
    return operands or [cleaned]


def _github_search_boolean_operator_count(query: str) -> int:
...
def _is_design_doc_path(lowered_path: str) -> bool:
    ...
    if name in {"design.md", "architecture.md", "patch_series.md"}:
        return True
    return any(
...
def _gh_json(cmd: list[str]) -> Any:
    result, _retryable, _successful = _gh_json_once(cmd, rate_limit_search=False)
    return result


def _gh_search_json(cmd: list[str]) -> Any:
    cached = _read_gh_search_cache(cmd)
    if cached is not None:
        return cached

    for attempt in range(2):
        result, retryable, successful = _gh_json_once(cmd, rate_limit_search=True)
        if successful:
            _write_gh_search_cache(cmd, result)
            return result
        if retryable and attempt == 0:
            _sleep_until_gh_search_reset()
            continue
        return result
    return []


def _gh_json_once(cmd: list[str], *, rate_limit_search: bool) -> tuple[Any, bool, bool]:
    ...
    try:
        return json.loads(completed.stdout or "[]"), False, True
...
def _acquire_gh_search_slot() -> None:
    while True:
        sleep_for = _reserve_gh_search_slot_or_delay()
        if sleep_for <= 0:
            return
...
def _reserve_gh_search_slot_or_delay() -> float:
    rate_dir = _gh_search_rate_directory()
    rate_dir.mkdir(parents=True, exist_ok=True)
    lock_path = rate_dir / "github-search-shared-rate-lock.file"
    ledger_path = rate_dir / "github-search-timestamps-ledger.jsonl"

    with lock_path.open("a+") as lock_file:
        _lock_gh_search_ledger(lock_file)
        try:
            now = time.time()
            cutoff = now - GH_SEARCH_WINDOW_SECONDS
            timestamps = [
                timestamp
                for timestamp in _read_gh_search_timestamps(ledger_path)
                if timestamp > cutoff
            ]
            ...
            if len(timestamps) < limit:
                ...
                _write_gh_search_timestamps(ledger_path, timestamps)
                return 0.0

            _write_gh_search_timestamps(ledger_path, timestamps)
            return timestamps[0] + GH_SEARCH_WINDOW_SECONDS - now
        finally:
            fcntl.flock(lock_file.fileno(), fcntl.LOCK_UN)


def _lock_gh_search_ledger(lock_file: Any) -> None:
    deadline = time.monotonic() + _env_float(
        "AI_ORG_GH_SEARCH_FILE_LOCK_TIMEOUT_SECONDS",
        GH_SEARCH_FILE_LOCK_TIMEOUT_SECONDS,
    )
    while True:
        try:
            ...
            if time.monotonic() >= deadline:
                raise TimeoutError("timed out waiting for GitHub search rate ledger lock")
            time.sleep(0.01)


def _read_gh_search_timestamps(path: Path) -> list[float]:
    try:
...
def _write_gh_search_timestamps(path: Path, timestamps: list[float]) -> None:
    tmp_path = path.with_name(f"{path.name}.tmp.{os.getpid()}")
    tmp_path.write_text("".join(f"{timestamp:.6f}\n" for timestamp in timestamps), encoding="utf-8")
    tmp_path.replace(path)


def _gh_search_rate_directory() -> Path:
    return DEFAULT_STORE_PATH.parent / "gh-search-rate"
...
def _sleep_until_gh_search_reset() -> None:
    reset_at = _gh_search_reset_epoch_seconds()
    max_wait = _env_float("AI_ORG_GH_SEARCH_RATE_LIMIT_MAX_WAIT_SECONDS", GH_SEARCH_RATE_LIMIT_MAX_WAIT_SECONDS)
    if reset_at is None:
        time.sleep(max_wait)
        return

    wait_for = max(0.0, reset_at - time.time()) + _gh_search_reset_jitter_seconds()
    time.sleep(min(wait_for, max_wait))


def _gh_search_reset_epoch_seconds() -> float | None:
    completed = _run_gh(["gh", "api", "rate_limit"])
    if completed is None or completed.returncode != 0:
        return None
    try:
    ...
    search = resources.get("search") if isinstance(resources, Mapping) else None
    if not isinstance(search, Mapping):
        return None
    try:
        reset_at = float(search.get("reset"))
    except (TypeError, ValueError):
        return None
    return reset_at if reset_at > 0 else None


def _gh_search_reset_jitter_seconds() -> float:
    return random.uniform(0.1, 1.0)


def _gh_search_cache_ttl_seconds() -> float:
    return _env_float("AI_ORG_GH_SEARCH_CACHE_TTL_SECONDS", GH_SEARCH_CACHE_TTL_SECONDS)


def _gh_search_cache_key(cmd: list[str]) -> str | None:
    if not _is_gh_search_command(cmd) or len(cmd) < 4:
        return None
    return json.dumps(cmd, separators=(",", ":"), ensure_ascii=False)


def _gh_search_cache_query(cmd: list[str]) -> str:
    return str(cmd[3]) if len(cmd) > 3 else ""


def _read_gh_search_cache(cmd: list[str]) -> Any | None:
    # Memento: the read-disabled switch bypasses memo-cache READS so a run
    # behaves as if the store knows nothing, but _write_gh_search_cache and
    # the gh search rate limiter stay fully active - everything learned is
    # banked and the rate shield never drops.
    if _precedent_read_disabled():
        return None
    cache_key = _gh_search_cache_key(cmd)
    if cache_key is None:
        return None
    ttl = _gh_search_cache_ttl_seconds()
    if ttl <= 0:
        return None
    ...
    try:
        ...
        try:
            ...
            row = connection.execute(
                """
                SELECT response_json, captured_at
                FROM gh_search_response_cache
                WHERE command_key = ?
                """,
                (cache_key,),
            ).fetchone()
    ...
    if row is None:
        return None
    if time.time() - float(row["captured_at"]) >= ttl:
        return None
    try:
...
def _write_gh_search_cache(cmd: list[str], response: Any) -> None:
    cache_key = _gh_search_cache_key(cmd)
    if cache_key is None:
        return
    try:
    ...
    try:
        ...
        try:
            ...
            with connection:
                connection.execute(
                    """
                    INSERT INTO gh_search_response_cache(
                        command_key,
                        search_kind,
                        query,
                        captured_at,
                    ...
                    """,
                    (cache_key, cmd[2], _gh_search_cache_query(cmd), time.time(), response_json),
                )
        finally:
            connection.close()
    except (OSError, sqlite3.Error, ValueError):
...
def _run_gh(cmd: list[str]) -> subprocess.CompletedProcess[str] | None:
    log_ctx = _precedent_ctx("engineering_precedent_store.gh")
    try:
...
def _codex_delta_inclusion(
    ...
    try:
        ...
        _record_precedent_timeout(context, exc)
        return {"keep": False, "reason": "precedent-store Codex timed out"}
...
def _codex_extract_pattern(
    ...
    try:
        ...
        return {
            ...
            "lang_env_version": "",
            "pitfalls": "",
            "evidence_class": UNCLASSIFIED_EVIDENCE_CLASS,
        }
...
def _codex_distill_candidate(
    ...
    if all(isinstance(result.get(field), str) for field in build_distill_schema()["properties"]):
        return result
    return {"snippet": "", "summary": "", "lang_env_version": "", "pitfalls": ""}
...
def _codex_author_level(term: str, context: Mapping[str, Any], candidate: Mapping[str, Any]) -> dict[str, Any]:
    try:
        result = _codex_json(
            ...
            build_author_level_schema(),
            "author-level.json",
        )
    except ReferenceCodexTimeout as exc:
        _record_precedent_timeout(context, exc)
        return {"author_level": "unknown", "reason": "precedent-store Codex timed out"}
...
def _codex_design_competence(term: str, context: Mapping[str, Any], candidate: Mapping[str, Any]) -> dict[str, Any]:
    try:
        result = _codex_json(
            ...
            build_design_competence_schema(),
            "design-competence.json",
        )
    except ReferenceCodexTimeout as exc:
        _record_precedent_timeout(context, exc)
        return {"keep": False, "author_level": "unknown", "reason": "precedent-store Codex timed out"}
    if (
...
def _extract_precedent_terms(text: str, context: Mapping[str, Any]) -> list[str]:
    try:
        result = _codex_json(
            _precedent_terms_prompt(text, context),
            build_precedent_terms_schema(),
            "precedent-terms.json",
        )
    except ReferenceCodexTimeout as exc:
        _record_precedent_timeout(context, exc)
        return []
    return _clean_precedent_terms(result.get("terms"))


def _codex_json(prompt: str, schema: Mapping[str, Any], output_name: str) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="ai-org-precedent-store-codex-") as tmp:
        temp_dir = Path(tmp)
        schema_file = temp_dir / "schema.json"
        out_file = temp_dir / output_name
        schema_file.write_text(json.dumps(schema), encoding="utf-8")
        timeout_seconds = _precedent_search_timeout_seconds()
        cmd = [
            "codex",
            "exec",
            "--sandbox",
        ...
        log_ctx = _precedent_ctx("engineering_precedent_store.codex").child(step=output_name)
        try:
...
def _search_keywords_prompt(term: str, context: Mapping[str, Any]) -> str:
    return (
        ...
        "that contain this target term's implementation pattern. Keep the precedent target term specific, "
...
def _design_search_keywords_prompt(term: str, context: Mapping[str, Any]) -> str:
    return (
        ...
        "concept. This is a separate design lane, not a code search. Prefer queries that find ADRs, patch seriess, "
        ...
        f"Precedent target term: {term}\n"
        f"Consuming stack context: {_json_for_prompt(context)}\n"
        "Rules: English only; include architecture, design, pattern, ADR, patch series, PEP, KEP, or decision words "
...
def _extract_prompt(term: str, context: Mapping[str, Any], repo: str, path: str, content: str) -> str:
    return (
        ...
        f"{_evidence_class_prompt_rules()}\n"
        f"Precedent target term: {term}\n"
        f"Consuming stack context: {_json_for_prompt(context)}\n"
        f"Repository: {repo}\n"
        f"Path: {path}\n"
...
def _design_web_sources_prompt(keywords: list[str], context: Mapping[str, Any]) -> str:
    return (
        ...
        "queries. Prefer accepted or merged patch seriess, PEPs, KEPs, ADRs, project architecture docs, pattern "
...
def _evidence_class_prompt_rules() -> str:
    return (
        "Classify evidence_class by SOURCE TYPE only, never by whether the content sounds like a workaround. "
        "Use standard for patch series/ISO/IEEE/W3C/PEP-level prescriptive documents; official_doc for vendor or "
        "project official documentation; peer_reviewed for published research; practitioner_report for "
        "engineering blogs, conference talks, postmortems, or ordinary repository implementation evidence; "
        "workaround_thread for issue comments, Q&A answers, forum posts, and ad hoc workaround threads."
    )
...
def _design_competence_prompt(term: str, context: Mapping[str, Any], candidate: Mapping[str, Any]) -> str:
    return (
        ...
        "for patch seriess, PEPs, KEPs, and ADRs; author or project domain track record; evidence of production use, "
...
def _distill_prompt(
    ...
    return (
        "Distill this kept candidate into a stored implementation precedent. The summary must be real: "
...
def _precedent_terms_prompt(text: str, context: Mapping[str, Any]) -> str:
    return (
        "Extract the implementation-bearing precedent terms from this patch series text for an implementation "
...
def _write_entry(entry: Mapping[str, Any]) -> None:
    if not _valid_entry(entry):
        raise ValueError("invalid precedent entry")
    path = _database_path()
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
        with _REFERENCE_DB_WRITE_LOCK:
            with _connect_database(path) as connection:
                ...
                with connection:
                    ...
                    # Memento: writes store term verbatim (write-exact, keys
                    # never destroyed). Only the DERIVED token index below is
                    # maintained for paraphrase-tolerant reads; stored rows stay as-is.
                    _index_term_tokens(connection, term, term_key)
                    for candidate in entry["candidates"]:
                        ...
                        existing = connection.execute(
                            ...
                            SELECT 1
                            FROM candidates
                            WHERE term_key = ? AND source_url = ? AND snippet = ? AND deprecated_at IS NULL
                            LIMIT 1
                            """,
...
def _deprecate_rows(term: str, reason: str, *, include_research: bool) -> dict[str, Any]:
    ...
    path = _existing_database_path()
    term_key = _normalize_term(clean_term)
    deprecated_at = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    op = "deprecate_term" if include_research else "deprecate_candidates"
    research_count = 0
    candidate_count = 0
    try:
        with _REFERENCE_DB_WRITE_LOCK:
                ...
                with connection:
                    if include_research:
                        cursor = connection.execute(
                            ...
                            WHERE term_key = ? AND deprecated_at IS NULL
                            """,
                            (deprecated_at, clean_reason, term_key),
                        )
                        research_count = int(cursor.rowcount if cursor.rowcount is not None else 0)
                    cursor = connection.execute(
                        """
                        UPDATE candidates
                        SET deprecated_at = ?, deprecated_reason = ?
                    ...
                    candidate_count = int(cursor.rowcount if cursor.rowcount is not None else 0)
                    connection.execute(
                        """
                        INSERT INTO reference_history(ts, op, term, detail)
                        VALUES (?, ?, ?, ?)
                        """,
                        (
                            time.time(),
...
def _read_entry(
    ...
    *,
    ...
    # Memento: AI_ORG_PRECEDENT_READ_DISABLED=1 makes every knowledge read
    # report a miss (emergence mode). Writes are never gated on this flag.
    if _precedent_read_disabled():
        return None
    path = _database_path()
    if not path.exists():
        if missing_ok:
            return None
        raise StoreUnavailable("missing", str(path))
    _raise_if_unreadable_path(path)
    term_key = _normalize_term(term)
    try:
        with _connect_existing_database(path) as connection:
            # Memento: exact key resolution first (short-circuit); only an
            # exact miss consults the derived paraphrase token index. The
            # fallback reads only - it never writes and never rewrites keys.
            rows = _fetch_entry_rows(connection, term_key, include_deprecated)
            if rows is None and paraphrase:
                for candidate_key in _paraphrase_candidate_keys(connection, term):
                    if candidate_key == term_key:
                        continue
                    rows = _fetch_entry_rows(connection, candidate_key, include_deprecated)
                    if rows is not None:
                        break
            if rows is None:
                return None
            research_rows, candidate_rows = rows
    except (OSError, sqlite3.Error) as exc:
        raise StoreUnavailable("unreadable", _store_error_detail(path, exc)) from exc
    ...
    try:
        ...
        entry = {
            ...
            "candidates": [_candidate_from_row(row, include_deprecation=include_deprecated) for row in candidate_rows],
            "notes": str(latest["notes"]),
        }
    except (TypeError, ValueError, json.JSONDecodeError) as exc:
        raise StoreUnavailable("unreadable", _store_error_detail(path, exc)) from exc
    if not _valid_persisted_entry(entry, research_history):
        raise StoreUnavailable("unreadable", f"{path}: invalid precedent store record")
...
def _fetch_entry_rows(
...
) -> tuple[list[sqlite3.Row], list[sqlite3.Row]] | None:
    active_research_clause = "" if include_deprecated else "AND deprecated_at IS NULL"
    active_candidate_clause = "" if include_deprecated else "AND deprecated_at IS NULL"
    ...
    candidate_rows = connection.execute(
        ...
        SELECT term, kind, snippet, summary, pitfalls, structure, rationale, when_to_use,
            ...
            evidence, delta_claim, lang_env_version, author_level, source_url, found_via, evidence_class,
            deprecated_at, deprecated_reason
        FROM candidates
        WHERE term_key = ?
        {active_candidate_clause}
        ORDER BY id
        """,
        (term_key,),
    ).fetchall()
    return research_rows, candidate_rows
...
def _ensure_schema(connection: sqlite3.Connection) -> None:
    ...
    with connection:
        ...
        _create_precedent_history_table(connection)
        _ensure_precedent_history_triggers(connection)
        _create_gh_search_response_cache_table(connection)
        _ensure_paraphrase_token_index(connection)

        connection.executescript(
            """
            CREATE INDEX IF NOT EXISTS idx_precedent_research_term ON research(term);
            CREATE INDEX IF NOT EXISTS idx_precedent_research_term_key ON research(term_key);
            CREATE INDEX IF NOT EXISTS idx_precedent_research_term_key_attempt ON research(term_key, attempt);
            CREATE INDEX IF NOT EXISTS idx_precedent_research_term_attempt ON research(term, attempt);
            CREATE INDEX IF NOT EXISTS idx_precedent_candidates_term ON candidates(term);
            CREATE INDEX IF NOT EXISTS idx_precedent_candidates_term_key ON candidates(term_key);
            CREATE INDEX IF NOT EXISTS idx_precedent_candidates_lang_env_version ON candidates(lang_env_version);
            CREATE INDEX IF NOT EXISTS idx_precedent_candidates_author_level ON candidates(author_level);
            CREATE INDEX IF NOT EXISTS idx_precedent_candidates_found_via ON candidates(found_via);
            CREATE INDEX IF NOT EXISTS idx_precedent_candidates_dedup ON candidates(term, source_url, snippet);
            CREATE INDEX IF NOT EXISTS idx_precedent_candidates_term_key_dedup ON candidates(term_key, source_url, snippet);
            CREATE INDEX IF NOT EXISTS idx_precedent_candidates_kind ON candidates(kind);
            CREATE INDEX IF NOT EXISTS idx_precedent_candidates_source_url ON candidates(term, source_url);
            CREATE INDEX IF NOT EXISTS idx_precedent_candidates_term_key_source_url ON candidates(term_key, source_url);
            CREATE INDEX IF NOT EXISTS idx_precedent_history_term ON reference_history(term);
            CREATE INDEX IF NOT EXISTS idx_precedent_history_op ON reference_history(op);
            CREATE INDEX IF NOT EXISTS idx_gh_search_response_cache_kind_query ON gh_search_response_cache(search_kind, query);
            CREATE INDEX IF NOT EXISTS idx_gh_search_response_cache_captured_at ON gh_search_response_cache(captured_at);
            CREATE INDEX IF NOT EXISTS idx_knowledge_term_tokens_term_key ON knowledge_term_tokens(term_key);
            """
        )
...
def _add_missing_research_columns(connection: sqlite3.Connection, research_columns: set[str]) -> None:
    ...
    if "deprecated_at" not in research_columns:
        connection.execute("ALTER TABLE research ADD COLUMN deprecated_at TEXT")
    if "deprecated_reason" not in research_columns:
        connection.execute("ALTER TABLE research ADD COLUMN deprecated_reason TEXT")
...
def _create_precedent_history_table(connection: sqlite3.Connection) -> None:
    connection.execute(
        """
...
def _ensure_precedent_history_triggers(connection: sqlite3.Connection) -> None:
    connection.executescript(
        """
        CREATE TRIGGER IF NOT EXISTS reference_history_after_research_insert
        ...
        BEGIN
            ...
            VALUES (
                ...
                'insert_research',
                NEW.term,
                printf('attempt=%d', NEW.attempt)
            );
        END;

        CREATE TRIGGER IF NOT EXISTS reference_history_after_candidate_insert
        ...
        BEGIN
            ...
            VALUES (
                ...
                'insert_candidate',
                NEW.term,
                printf('kind=%s;source_url=%s', NEW.kind, NEW.source_url)
            );
        END;
        """
    )
...
def _create_gh_search_response_cache_table(connection: sqlite3.Connection) -> None:
    connection.execute(
        """
        CREATE TABLE IF NOT EXISTS gh_search_response_cache (
...
_PARAPHRASE_INDEX_COLUMNS = {"token", "kind", "source_token", "term_key"}
_PARAPHRASE_RULES_VERSION_META_KEY = "paraphrase_family_rules_version"


def _ensure_paraphrase_token_index(connection: sqlite3.Connection) -> None:
    """Keep the derived paraphrase token index in sync with the frozen rules.

    Memento (brief 30): the index is a REBUILDABLE DERIVED layer over the
    verbatim term columns - research/candidates rows are provenance and are
    never touched here (keys byte-untouched). The family rule list is
    version-stamped in knowledge_index_meta; a stamp mismatch (including a
    pre-family store with no stamp, or a legacy two-column index) drops and
    fully rebuilds ONLY the derived table, so a future rule change can never
    leave stale family tokens behind. With a current stamp this is the
    idempotent additive backfill (safe on every open of a live store).
    """
    connection.execute(
        """
    ...
    stamped_row = connection.execute(
        "SELECT value FROM knowledge_index_meta WHERE key = ?",
        (_PARAPHRASE_RULES_VERSION_META_KEY,),
    ).fetchone()
    stamped = str(stamped_row["value"]) if stamped_row is not None else ""
    columns = _table_columns(connection, "knowledge_term_tokens")
    if columns and (stamped != PARAPHRASE_FAMILY_RULES_VERSION or columns != _PARAPHRASE_INDEX_COLUMNS):
        # Derived layer only: dropping it loses nothing, the backfill below
        # regenerates it from the verbatim term columns.
        connection.execute("DROP TABLE knowledge_term_tokens")
        columns = set()
    if not columns:
        _create_knowledge_term_tokens_table(connection)
    _backfill_knowledge_term_tokens(connection)
    connection.execute(
        "INSERT OR REPLACE INTO knowledge_index_meta(key, value) VALUES (?, ?)",
        (_PARAPHRASE_RULES_VERSION_META_KEY, PARAPHRASE_FAMILY_RULES_VERSION),
    )


def _create_knowledge_term_tokens_table(connection: sqlite3.Connection) -> None:
    # PROVISIONAL: a plain token table instead of SQLite FTS5, so the
    # paraphrase read layer works on any bundled sqlite build without a
    # runtime capability probe. The table is DERIVED and rebuildable from the
    # verbatim term columns; dropping it loses nothing and the next store open
    # backfills it. It must never be treated as source data.
    # kind: 'exact' = the original token itself; 'family' = the additive
    # morphological family token. source_token is always the stored ORIGINAL
    # token that emitted the row, so stored-side fractions stay over
    # originals.
    connection.execute(
        """
        CREATE TABLE IF NOT EXISTS knowledge_term_tokens (
            token TEXT NOT NULL,
            kind TEXT NOT NULL,
            source_token TEXT NOT NULL,
            term_key TEXT NOT NULL,
            PRIMARY KEY (token, kind, source_token, term_key)
        )
        """
    )


def _backfill_knowledge_term_tokens(connection: sqlite3.Connection) -> None:
    """Idempotently backfill the derived token index from existing keys.

    Memento: this migration is ADDITIVE ONLY. It runs on every store open
    (CREATE IF NOT EXISTS + insert missing index rows) and must never rewrite
    or delete research/candidates rows - the live org store holds hundreds of
    terms whose keys are provenance. Term keys already present in the index
    are skipped, and _write_entry keeps the index fresh for new writes.
    Family-rule changes are handled one level up: the version stamp check in
    _ensure_paraphrase_token_index rebuilds the whole derived table, because
    an additive pass alone would skip existing keys and strand their family
    rows on the old rules.
    """
    rows = connection.execute(
        """
        SELECT term, term_key FROM research
        WHERE term_key != '' AND term_key NOT IN (SELECT term_key FROM knowledge_term_tokens)
        UNION
        SELECT term, term_key FROM candidates
        WHERE term_key != '' AND term_key NOT IN (SELECT term_key FROM knowledge_term_tokens)
        """
    ).fetchall()
    for row in rows:
        _index_term_tokens(connection, str(row["term"] or ""), str(row["term_key"] or ""))


def _index_term_tokens(connection: sqlite3.Connection, term: str, term_key: str) -> None:
    if not term_key:
        return
    for token in _paraphrase_tokens(term):
        connection.execute(
            "INSERT OR IGNORE INTO knowledge_term_tokens(token, kind, source_token, term_key) VALUES (?, 'exact', ?, ?)",
            (token, token, term_key),
        )
        family = _paraphrase_family_token(token)
        if family:
            connection.execute(
                "INSERT OR IGNORE INTO knowledge_term_tokens(token, kind, source_token, term_key) VALUES (?, 'family', ?, ?)",
                (family, token, term_key),
            )
...
def _database_path() -> Path:
    raw = str(os.environ.get("AI_ORG_REFERENCE_STORE") or "").strip()
    # Memento: AI_ORG_REFERENCE_STORE started as a test-injection knob, then was
    # misused as primary wiring. The org asset path is the code default so
    # forgetting the override yields the real store, not a silent empty one.
    store = Path(raw).expanduser() if raw else DEFAULT_STORE_PATH
    resolved = store.resolve()
    if _path_is_inside(resolved, REPO_ROOT) or _path_is_inside(resolved, REPO_ROOT / ".git"):
        raise ValueError("AI Org precedent store must be outside the work repo and its .git directory")
    return resolved


def _existing_database_path() -> Path:
    path = _database_path()
    if not path.exists():
        raise StoreUnavailable("missing", str(path))
    _raise_if_unreadable_path(path)
...
def _raise_if_unreadable_path(path: Path) -> None:
    try:
        stat_result = path.stat()
    except OSError as exc:
        raise StoreUnavailable("unreadable", _store_error_detail(path, exc)) from exc
    if stat_result.st_mode & 0o444 == 0:
        raise StoreUnavailable("unreadable", f"{path}: no read permission bits")


def _store_error_detail(path: Path, exc: BaseException) -> str:
...
def _candidate_from_row(
    ...
    *,
    ...
    include_deprecation: bool = False,
) -> dict[str, str]:
    ...
    if kind == "design":
        candidate = {
            ...
            "evidence_class": _clean_evidence_class(_row_get(row, "evidence_class")),
        }
    else:
        candidate = {
            "kind": "implementation",
    ...
    if include_deprecation:
        deprecated_at = str(_row_get(row, "deprecated_at") or "")
        deprecated_reason = str(_row_get(row, "deprecated_reason") or "")
        if deprecated_at or deprecated_reason:
            candidate["deprecated_at"] = deprecated_at
            candidate["deprecated_reason"] = deprecated_reason
    if include_term:
...
def _research_attempt_from_row(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    attempt = {
        ...
        "notes": notes,
        "_precedent_kinds": kinds,
    }
    deprecated_at = str(_row_get(row, "deprecated_at") or "")
    deprecated_reason = str(_row_get(row, "deprecated_reason") or "")
    if deprecated_at or deprecated_reason:
        attempt["deprecated_at"] = deprecated_at
        attempt["deprecated_reason"] = deprecated_reason
...
def _split_research_kind_note(notes: str) -> tuple[str, tuple[str, ...]]:
    ...
    try:
        kinds = _precedent_kinds([value for value in raw_kinds if value])
    except ValueError:
...
def _research_attempt_kinds(attempt: Mapping[str, Any]) -> tuple[str, ...]:
    kinds = attempt.get("_precedent_kinds")
    ...
    if isinstance(research_history, list):
        for attempt in research_history:
            if isinstance(attempt, dict):
                attempt.pop("_precedent_kinds", None)
                attempt["notes"] = _split_research_kind_note(str(attempt.get("notes") or ""))[0]
    return public


def _empty_public_research_entry(term: str) -> dict[str, Any]:
    return {
        "term": str(term),
        "search_keywords": [],
        "examined": [],
...
def _preheld_candidate(
    ...
    if kind == "design":
        candidate = {
            ...
            "evidence_class": _clean_evidence_class(facet.get("evidence_class")),
        }
    else:
        candidate = {
            "kind": "implementation",
...
def _valid_persisted_entry(entry: Any, research_history: Any) -> bool:
    ...
    return (
        isinstance(candidates, list)
        and all(_valid_candidate(_candidate_without_deprecation(candidate)) for candidate in candidates)
        and all(candidate["found_via"].lower() in keyword_set for candidate in candidates)
    )
...
def _valid_research_attempt(value: Any) -> bool:
    ...
    optional_fields = {"deprecated_at", "deprecated_reason"}
    if not isinstance(value, Mapping) or not required_fields.issubset(value) or not set(value).issubset(
        required_fields | optional_fields
    ):
        return False
    if ("deprecated_at" in value) != ("deprecated_reason" in value):
        return False
    if "deprecated_at" in value and (
        not isinstance(value["deprecated_at"], str) or not isinstance(value["deprecated_reason"], str)
    ):
        return False
    if not isinstance(value["term"], str) or not isinstance(value["notes"], str):
        return False
    if not isinstance(value["_precedent_kinds"], tuple) or not all(
        kind in CANDIDATE_KINDS for kind in value["_precedent_kinds"]
    ):
        return False
...
def _candidate_without_deprecation(candidate: Mapping[str, Any]) -> dict[str, Any]:
    return {
        key: value
        for key, value in candidate.items()
        if key not in {"deprecated_at", "deprecated_reason"}
    }
...
def _valid_candidate(value: Any) -> bool:
    ...
    allowed_evidence_classes = set(EVIDENCE_CLASSES) | {UNCLASSIFIED_EVIDENCE_CLASS}
    return (
        set(normalized) == set(fields)
        and all(isinstance(normalized[field], str) for field in fields)
        and normalized["evidence_class"] in allowed_evidence_classes
    )
...
def _normalize_raw_candidate(value: Any, search_keywords: list[str] | None = None) -> dict[str, str] | None:
    ...
    if not snippet:
        return None
    evidence_class = _clean_evidence_class(value.get("evidence_class"))
...
def _fold_workaround_candidates(
    term: str,
    ...
    non_workarounds = [
        dict(candidate)
        for candidate in candidates
        if candidate.get("evidence_class") != "workaround_thread"
    ]
    workarounds = [
        candidate
        for candidate in candidates
    ...
    for workaround in workarounds:
        target = _strongest_workaround_fold_target(non_workarounds)
        if target is None:
            _emit_workaround_event("engineering_precedent_store.workaround_candidate.dropped", term, workaround, None, ctx)
            continue
        _append_workaround_exception(target, workaround)
        _emit_workaround_event("engineering_precedent_store.workaround_candidate.folded", term, workaround, target, ctx)
    return non_workarounds


def _strongest_workaround_fold_target(candidates: list[dict[str, str]]) -> dict[str, str] | None:
    if not candidates:
        return None
    return max(candidates, key=_candidate_strength_key)


def _candidate_strength_key(candidate: Mapping[str, Any]) -> tuple[int, int, int, str]:
    evidence_rank = {
        "standard": 5,
        "official_doc": 4,
        "peer_reviewed": 3,
        "practitioner_report": 2,
        UNCLASSIFIED_EVIDENCE_CLASS: 1,
    }
    author_rank = {
        "expert": 5,
        "high": 4,
    ...
    evidence = str(candidate.get("evidence_class") or UNCLASSIFIED_EVIDENCE_CLASS)
    author = str(candidate.get("author_level") or "unknown").strip().lower()
    kind_rank = 1 if _candidate_kind(candidate) == "design" else 0
    return (
        evidence_rank.get(evidence, 0),
        author_rank.get(author, 2),
        kind_rank,
        str(candidate.get("source_url") or ""),
    )


def _append_workaround_exception(target: dict[str, str], workaround: Mapping[str, str]) -> None:
    exception = _bounded_workaround_exception(workaround)
    if _candidate_kind(target) == "design":
        field = "alternatives"
    else:
        field = "pitfalls"
    existing = str(target.get(field) or "").strip()
    target[field] = f"{existing} {exception}".strip() if existing else exception


def _bounded_workaround_exception(workaround: Mapping[str, str]) -> str:
    detail = " ".join(
        str(workaround.get(field) or "").strip()
        for field in (
            "summary",
            "delta_claim",
            "structure",
        ...
        if str(workaround.get(field) or "").strip()
    )
    detail = " ".join(detail.split())[:480]
    source = str(workaround.get("source_url") or "").strip()
    if source:
        return f"Bounded workaround exception from {source}: {detail}".strip()
    return f"Bounded workaround exception: {detail}".strip()


def _emit_workaround_event(
    event_type: str,
    term: str,
    ...
    payload = {
        ...
        "candidate_found_via": str(candidate.get("found_via") or ""),
        "candidate_evidence_class": str(candidate.get("evidence_class") or ""),
    }
    if target is not None:
        payload.update(
            {
                "target_kind": _candidate_kind(target),
                "target_source_url": str(target.get("source_url") or ""),
                "target_evidence_class": str(target.get("evidence_class") or ""),
            }
        )
    org_log.emit(event_type, payload, ctx=ctx)
...
def _clean_evidence_class(value: Any) -> str:
    text = str(value or "").strip().lower()
    if not text:
        return UNCLASSIFIED_EVIDENCE_CLASS
    if text in EVIDENCE_CLASSES or text == UNCLASSIFIED_EVIDENCE_CLASS:
        return text
    return ""
...
def _raw_design_candidate(
    ...
    *,
    ...
    candidate = {
        ...
        "evidence_class": _clean_evidence_class(extracted.get("evidence_class")),
        "_precedent_repo": str(repo or "").strip(),
        "_precedent_language": str(language or "").strip(),
        "_precedent_found_via": str(found_via or "").strip(),
    }
...
def _candidate_audit_repo(candidate: Any) -> str:
    if not isinstance(candidate, Mapping):
        return ""
    repo = str(candidate.get("_precedent_repo") or "").strip()
    if repo:
        return repo
...
def _candidate_audit_language(candidate: Any) -> str:
    if not isinstance(candidate, Mapping):
        return ""
    language = str(candidate.get("_precedent_language") or "").strip()
    if language:
        return language
...
def _candidate_audit_found_via(candidate: Any, search_keywords: list[str]) -> str:
    ...
    return _clean_found_via(candidate.get("_precedent_found_via") or candidate.get("found_via"), search_keywords)
...
def _patch_series_text(value: Any) -> str:
    if isinstance(value, Mapping):
        return "\n".join(_patch_series_text(item) for item in value.values())
    if isinstance(value, list):
        return "\n".join(_patch_series_text(item) for item in value)
    return str(value or "")


def _clean_precedent_terms(values: Any) -> list[str]:
    if not isinstance(values, list):
        return []
    terms = []
    for value in values:
