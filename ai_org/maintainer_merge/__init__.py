...
import ai_org.maintainer_merge.mainline as mainline
import ai_org.maintainer_merge.subsystem as subsystem
...
def pull(repo):
    ...
    for branch in sorted(git_wrapper.branches(repo, f"{CONTRIB_PREFIX}*")):
        ...
        try:
            ...
            return {"accept": False, "ref": None, "reasons": [str(exc)]}
        # Migrated contributions are authorized from the immutable root named
        # by their task binding.  Only unbound historical contributions use
        # the announcement/branch compatibility blocker below.
        if not migrated:
            generation_blocker = _root_generation_blocker(repo, branch)
            if generation_blocker is not None:
                return generation_blocker
        if migrated and tip and evidence.has_verified_link(
            repo, tip, (SUBSYSTEM_BRANCH, MAINLINE_BRANCH)
        ):
            continue
        if not migrated and git_wrapper.is_ancestor(repo, branch, SUBSYSTEM_BRANCH):
            continue
...
def _root_generation_blocker(repo, contrib_branch: str):
    announcement = announcements.find_for_contrib(repo, contrib_branch)
    series_branch = ""
    if announcement is not None:
        series_branch = str(announcement["record"].get("series_branch") or "")
    if not series_branch:
        inferred = "ai-org/patch-series/" + contrib_branch.removeprefix(CONTRIB_PREFIX)
        if git_wrapper.branch_exists(repo, inferred):
            series_branch = inferred
    if not series_branch:
        return None
    snapshot = patch_series_bodies.classify_root_generation(repo, series_branch)
    if not snapshot.lifecycle_blocked:
        return None
    return {
        "ok": False,
