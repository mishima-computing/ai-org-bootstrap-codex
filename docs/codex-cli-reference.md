...
- **`--output-schema <FILE>`** — a JSON Schema the final message must match → forces *structured* output.
  *Use:* deterministic parsing — reviewer verdicts, lineage splits, acceptance results. No regex on prose.
...
| `rfc/lineage` | read-only | repo | `--output-schema` (lineage split) + `-o` |
