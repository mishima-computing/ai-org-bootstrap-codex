...
def test_submit_accepts_plain_json_string_and_json_file_with_collision_ids(tmp_path):
    ...
    request_file = tmp_path / "maintainer-series-request.json"
...
def test_submit_module_reports_argv_errors_cleanly(tmp_path):
    result = subprocess.run(
        [sys.executable, "-m", "ai_org.patchwork_queue.submit"],
    ...
    assert "usage: python -m ai_org.patchwork_queue.submit" in result.stderr
