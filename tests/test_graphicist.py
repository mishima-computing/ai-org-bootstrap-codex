...
def test_constructive_svg_prompt_injects_cached_precedent_canons(monkeypatch):
    calls = []

    def fake_lookup(term):
        calls.append(term)
        return f"- structure: distilled canon for {term}"

    monkeypatch.setattr(graphicist, "_lookup_precedent_canon", fake_lookup)
    graphicist._REFERENCE_CANON_CACHE.clear()

    spec = {"subject": "LPC character sprite with cape", "target_head_units": 3.5}
    prompt = graphicist._constructive_svg_prompt(spec, style="cute")
    second_prompt = graphicist._constructive_svg_prompt(spec, style="cute")

    assert "construction axioms (source-backed logo/mascot doctrine - apply as geometry)" in prompt
    for term in graphicist.CONSTRUCTION_CANON_TERMS:
        assert f"[{term}]" in prompt
    assert "[LPC style guide existence proof]" in prompt
    assert "[art bible two layers plus exemplar anchor]" in prompt
    assert "[doctrine limits judgment override]" not in prompt
    assert "- structure: distilled canon for vector construction grid doctrine" in prompt
    assert "- structure: distilled canon for vector silhouette first construction" in prompt
    deformation_terms = (
        "deformation information redistribution",
        "identity invariants under deformation",
        "chibi template grammar and slots",
    )
    for term in deformation_terms:
        assert f"[{term}]" in prompt
        assert f"- structure: distilled canon for {term}" in prompt
    assert second_prompt == prompt
    assert calls.count("vector construction grid doctrine") == 1
    assert calls.count("vector silhouette first construction") == 1
    for term in deformation_terms:
        assert calls.count(term) == 1
    assert "doctrine limits judgment override" not in calls


def test_critique_canon_block_includes_critique_only_doctrine(monkeypatch):
    calls = []
    ...
    block = graphicist._critique_canon_block({"subject": "logo mascot"}, style="cute")

    assert "construction axioms (source-backed logo/mascot doctrine - apply as geometry)" in block
    assert "critique doctrine (source-backed judgment limits)" in block
    assert "[doctrine limits judgment override]" in block
    assert calls.count("doctrine limits judgment override") == 1


def test_precedent_canon_injection_fail_opens_and_logs_store_unavailable(monkeypatch):
    events = []

    def unavailable_lookup(*args, **kwargs):
        raise engineering_precedent_store.StoreUnavailable("unset", "test store unset")

    def emit(event_type, payload, **kwargs):
        events.append((event_type, payload, kwargs))
        return {"event_id": "test-event"}

    monkeypatch.setattr(engineering_precedent_store, "lookup", unavailable_lookup)
    monkeypatch.setattr(graphicist.org_log, "emit", emit)
    graphicist._REFERENCE_CANON_CACHE.clear()

    prompt = graphicist._constructive_svg_prompt({"subject": "cute mascot"}, style="cute")

    assert "COMPOSE in this requested view" in prompt
    assert events
    assert all(event[0] == "engineering_precedent_store.store_unavailable" for event in events)
    assert all(event[1] == {"reason": "unset", "caller": "graphicist._lookup_precedent_canon"} for event in events)


def test_spec_and_prompt_include_body_yaw_and_head_unit_targets():
    brief = {
        "subject": "appeal hero portrait",
        "defining_features": ["cape", "boots"],
        "realism_cute": 1.0,
        "style": "cute",
        "view": "three-quarter",
        "palette_hint": "red and gold",
        "canon_notes": "Readable silhouette.",
        "animation": {"needed": False, "states": []},
    }

    spec = graphicist._spec_from_brief("hero", brief)
    prompt = graphicist._constructive_svg_prompt(spec, view="three-quarter", style="cute")

    assert spec["yaw_degrees"] == 30.0
    assert spec["target_head_units"] == 3.5
    assert "Body-yaw construction:" in prompt
    assert "Target body yaw: 30 degrees" in prompt
    assert "near shoulder/hip/foot" in prompt
    assert "Head-count construction:" in prompt
    assert "Target full figure height: 3.5 head units" in prompt


def test_spec_uses_requested_head_units_as_contract_over_style_default():
    brief = {
        "subject": "appeal chibi hero",
        "defining_features": ["oversized head", "tiny body"],
        "realism_cute": 1.0,
        "style": "cute",
        "view": "front",
        "requested_head_units": 2.0,
        "palette_hint": "red and gold",
        "canon_notes": "Requester specified exactly 2.0 heads.",
        "animation": {"needed": False, "states": []},
    }

    spec = graphicist._spec_from_brief("exactly 2.0 heads chibi hero", brief)

    assert spec["requested_head_units"] == 2.0
    assert spec["target_head_units"] == 2.0
    assert graphicist._target_head_units(spec, style="cute") == 2.0


def test_spec_uses_realism_default_when_head_units_not_requested():
    brief = {
        "subject": "appeal hero portrait",
        "defining_features": ["cape", "boots"],
        "realism_cute": 1.0,
        ...
        "view": "front",
        "palette_hint": "red and gold",
        "canon_notes": "No requested head-count proportion.",
        "animation": {"needed": False, "states": []},
    }

    spec = graphicist._spec_from_brief("cute hero", brief)

    assert spec["requested_head_units"] is None
    assert spec["target_head_units"] == 3.5


def test_measure_svg_head_unit_count_from_synthetic_groups():
    svg = (
        '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">'
        '<g id="body"><rect x="156" y="80" width="200" height="320"/></g>'
        '<g id="head"><ellipse cx="256" cy="120" rx="50" ry="40"/></g>'
        "</svg>"
    )

    measurement = graphicist._measure_svg_head_unit_count(svg)

    assert measurement["ok"] is True
    assert measurement["head_bbox"] == [206.0, 80.0, 306.0, 160.0]
    assert measurement["figure_bbox"] == [156.0, 80.0, 356.0, 400.0]
    assert measurement["head_units"] == pytest.approx(4.0)


def test_head_count_correction_regenerates_with_measured_feedback(monkeypatch, tmp_path):
    svg_path = tmp_path / "asset.svg"
    png_path = tmp_path / "asset.png"
    png_path.write_bytes(b"rendered")
    svg_path.write_text(
        '<svg viewBox="0 0 512 512">'
        '<g id="body"><rect x="156" y="80" width="200" height="320"/></g>'
        '<g id="head"><rect x="206" y="80" width="100" height="80"/></g>'
        "</svg>",
        encoding="utf-8",
    )
    captured = {}
    ...
    def fake_constructive(spec, out_path, model=None, view="three-quarter", style="painterly"):
        ...
        out_path.write_text("<svg viewBox=\"0 0 512 512\"><g id=\"head\"/></svg>", encoding="utf-8")
        return out_path

    monkeypatch.setattr(graphicist, "constructive_svg", fake_constructive)

    corrected = graphicist._correct_head_count_after_render(
        {"subject": "cute hero head", "target_head_units": 3.5},
        svg_path,
        png_path,
        view="three-quarter",
        style="cute",
    ...
    assert captured["view"] == "three-quarter"
    assert captured["style"] == "cute"
    assert captured["spec"]["head_unit_feedback"]["measured_head_units"] == pytest.approx(4.0)
    assert captured["spec"]["head_unit_feedback"]["target_head_units"] == 3.5
    assert report["decision"] == "corrected"
    assert report["measured_head_units"] == pytest.approx(4.0)
    assert report["target_head_units"] == 3.5
    assert log_lines == [
        "head-unit QA: measured=4.000; target=3.500; outside tolerance, correction applied"
    ]


def test_autonomous_create_returns_head_unit_measurement_log(monkeypatch, tmp_path):
    brief = {
        "subject": "contract chibi hero",
        "defining_features": ["two-head proportion", "round head"],
        "realism_cute": 1.0,
        "style": "cute",
        "view": "front",
        "requested_head_units": 2.0,
        "palette_hint": "blue and white",
        "canon_notes": "Exactly 2.0 heads.",
        "animation": {"needed": False, "states": []},
    }
    calls = {"constructive": 0}

    def fake_research(_request):
        return brief

    def fake_constructive(_spec, out_path, model=None, view="three-quarter", style="painterly"):
    ...
    monkeypatch.setattr(graphicist, "_research_art_brief", fake_research)
    monkeypatch.setattr(graphicist, "constructive_svg", fake_constructive)
    monkeypatch.setattr(graphicist, "render_svg", fake_render)
    monkeypatch.setattr(graphicist, "_critique_rendered_asset", lambda *args, **kwargs: {"matches": True, "svg": None})

    result = graphicist.autonomous_create("draw exactly 2.0 heads", tmp_path)

    assert calls["constructive"] == 2
    assert result["head_unit_qa"]["decision"] == "corrected"
    assert result["head_unit_qa"]["target_head_units"] == 2.0
    assert result["head_unit_qa"]["measured_head_units"] == pytest.approx(4.0)
    assert result["log_lines"] == [
        "head-unit QA: measured=4.000; target=2.000; outside tolerance, correction applied"
    ]
