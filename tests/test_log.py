...
def test_projections_rebuild_from_log_alone_and_tail_cli(tmp_path):
    ...
    org_log.emit("patch_series.produce.completed", {"duration_seconds": 1.5}, ctx=ctx)
    ...
    org_log.emit("patch_series.pull.outcome", {"status": "needs_work", "failed_step": "grounding"}, ctx=ctx)
...
def test_projection_counts_step_lifecycle_when_completed_payload_overflows(tmp_path):
    ctx = org_log.RunContext(repo=tmp_path, run_id="run-20260703T010209Z-steps")
    big_tree = {"body": "x" * (org_log.HARD_PAYLOAD_BYTES + 1)}

    first = ctx.child(stage="approach", step="normalize_problem")
    org_log.emit("approach.step.started", {"step": "normalize_problem"}, ctx=first)
    org_log.emit(
        "approach.step.completed",
        {"step": "normalize_problem", "duration_seconds": 1.0, "technical_approach": big_tree, "current_step": "extract_constraints"},
        ctx=first,
    )
    second = ctx.child(stage="approach", step="extract_constraints")
    org_log.emit("approach.step.started", {"step": "extract_constraints"}, ctx=second)
    org_log.emit(
        "approach.step.completed",
        {"step": "extract_constraints", "duration_seconds": 2.0, "technical_approach": big_tree, "current_step": None},
        ctx=second,
    )

    projections = org_log.rebuild_projections(tmp_path, ctx.run_id)

    assert projections["progress_snapshot"]["progress"]["steps_done"] == ["normalize_problem", "extract_constraints"]
    assert projections["progress_snapshot"]["current_step"] is None
    assert "steps=2 current=-" in projections["canonical_run_line"]["line"]
